...
export default function TeacherDashboard() {
  ...
  function rejectUser(email, name) {
    ...
    <div className="page-teacher">
      ...
      <div className="app-container">
        <div style={{ paddingTop: 20 }}>
          ...
          <div
            className="card"
            style={{ background: 'linear-gradient(135deg,#f0d9ff,#d6f5ec)', marginBottom: 20 }}
          >
          ...
          {pendingUsers.length > 0 && (
            <div
              className="card"
              style={{
                background: 'linear-gradient(135deg,#fff8cc,#ffe8d0)',
                borderColor: '#ffd060',
              }}
            >
          ...
          <div className="teacher-grid">
            ...
            <div className="card">
              <div className="card-title">
                <span className="emoji">⭐</span> 학생 포인트 관리
              </div>
              {displayStudents.length === 0 ? (
                ...
                displayStudents.map((s) => (
                  <div key={s.id} className="student-row">
                    ...
                    <button
                      className="add-pts-btn"
                      onClick={() => setPtsTarget({ id: s.id, name: s.name })}
                    >
                      ➕ 포인트
                    </button>
                  </div>
            ...
            <div className="card">
              <div className="card-title">
                <span className="emoji">🔒</span> 차시 관리
              </div>
                ...
                {SESSIONS.map((s, i) => (
                  <div key={s.id} className="session-manage-row">
                    ...
                    <div className="session-manage-info">
                      <div className="session-manage-name">
                        {s.sport} × {s.skill}
                      </div>
                      <div className="session-manage-sport">
                        {s.skillEmoji} {s.desc}
                      </div>
                    </div>
                    <button
                      className={`unlock-btn${sessionLocks[i] ? ' locked-btn' : ''}`}
            ...
            <div className="card">
              <div className="card-title">
                <span className="emoji">🏆</span> 전체 순위
              </div>
              {sortedStudents.map(([id, sd], i) => (
                <div key={id} className="rank-item">
                  ...
                  <div
                    ...
                    style={{ background: `var(--${RANK_COLORS[i % RANK_COLORS.length]}-light)` }}
                  >
                    {sd.emoji}
                  </div>
            ...
            <div className="card">
              <div className="card-title">
                <span className="emoji">📋</span> 포인트 지급 내역
              </div>
                    ...
                    <div key={i} className="history-item">
                      ...
                      <div className="history-info">
                        <div className="history-name">
                          {h.name} · {h.note || h.action || h.reason || ''}
                        </div>
